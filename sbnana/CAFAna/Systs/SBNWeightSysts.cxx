...
/*
    std::cout << "[JSKIMDEBUG][SBNWeightUnivSyst::Shift] idx_left = " << idx_left << std::endl;
    std::cout << "[JSKIMDEBUG][SBNWeightUnivSyst::Shift] idx_right = " << idx_right << std::endl;
*/
    double rw_left = vec_TruthVar[idx_left](nu);
    double rw_right = vec_TruthVar[idx_right](nu);
/*
    std::cout << "[JSKIMDEBUG][SBNWeightUnivSyst::Shift] rw_left = " << rw_left << std::endl;
    std::cout << "[JSKIMDEBUG][SBNWeightUnivSyst::Shift] rw_right = " << rw_right << std::endl;
*/
    double sigma_left = univ_to_sigma[idx_left].second;
    double sigma_right = univ_to_sigma[idx_right].second;
/*
    std::cout << "[JSKIMDEBUG][SBNWeightUnivSyst::Shift] sigma_left = " << sigma_left << std::endl;
    std::cout << "[JSKIMDEBUG][SBNWeightUnivSyst::Shift] sigma_right = " << sigma_right << std::endl;
*/
    double this_rw = rw_left + (x-sigma_left) * (rw_left - rw_right) / (sigma_left - sigma_right);
    if(this_rw<0) this_rw = 0.;
    weight *= this_rw;
/*
    std::cout << "[JSKIMDEBUG][SBNWeightUnivSyst::Shift] -> weight = " << rw_left + (x-sigma_left) * (rw_left - rw_right) / (sigma_left - sigma_right) << std::endl;
*/
  }
