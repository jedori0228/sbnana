...
namespace ana
{
  ...
  //----------------------------------------------------------------------
  TGraphAsymmErrors* EnsembleSpectrum::ErrorBand(double q,
                                                 double exposure,
                                                 ...
                                                 EBinType bintype) const
  {
    ...
    for(int binIdx = 0; binIdx < hnom->GetNbinsX()+2; ++binIdx){
      ...
      const double y0 = FindQuantile( ROOT::Math::gaussian_cdf_c(q,1) , ys);
      const double y1 = FindQuantile( ROOT::Math::gaussian_cdf(q,1), ys);
